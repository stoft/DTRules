...
public class Rules2Excel {
  ...
  private void writeDT(RDecisionTable dt) {
    if (balanced) {
      maxCol =
        dt.getActionTableBalanced(session).length > 0
          ? dt.getActionTableBalanced(session)[0].length
          : 0;
  ...
  int writeName(RDecisionTable dt, Sheet s, int cRow) {
    ...
    setCell(
      ...
      c,
      "Name: " + name,
      s.getColumnWidth(1) + s.getColumnWidth(2) + s.getColumnWidth(3) * maxCol
    );
  ...
  int writeFields(RDecisionTable dt, Sheet s, int cRow) {
    ...
    for (RName field : fieldnames) {
      ...
      setCell(
        r,
        c,
        field.stringValue() + ": " + fields.get(field),
        s.getColumnWidth(1) + s.getColumnWidth(2) + s.getColumnWidth(3) * maxCol
      );
    ...
    //CONTEXTS:
    {
      ...
      for (int cnt = 1; cnt <= contexts.length; cnt++) {
        ...
        setCell(
          ...
          c,
          contexts[cnt - 1],
          s.getColumnWidth(2) + s.getColumnWidth(3) * maxCol
        );
  ...
  int writeConditions(RDecisionTable dt, Sheet s, int cRow) {
    ...
    String conditionTable[][] = balanced
      ? dt.getConditionTableBalanced(session)
      : dt.getConditiontable();
  ...
  int writeActions(RDecisionTable dt, Sheet s, int cRow) {
    ...
    String actionTable[][] = balanced
      ? dt.getActionTableBalanced(session)
      : dt.getActiontable();
  ...
  int writePolicyStatements(RDecisionTable dt, Sheet s, int cRow) {
    ...
    String policystatements[] = balanced
      ? dt.getPolicyStatementsBalanced(session)
      : dt.getPolicystatements();
  ...
  int writeEntity(Sheet s, int rowC, RName entity) {
    ...
    ArrayList<RName> attributes = EntityFactory.sorted(
      e.getAttributeIterator(),
      true
    );
  ...
  void writeEDD(String excelName) {
    try {
      ...
      ArrayList<RName> list = EntityFactory.sorted(
        ef.getEntityRNameIterator(),
        true
      );
      int rowC = 1;
  ...
  public void writeExcel(
    ...
    try {
      ...
      writeDecisionTables(
        ruleset.getWorkingdirectory() + excelName,
        fields,
        ascending,
        limit
      );
