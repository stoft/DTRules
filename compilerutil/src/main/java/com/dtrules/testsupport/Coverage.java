...
public class Coverage {
  ...
  public class TraceLoad implements IGenericXMLParser {
    ...
    void pushDT(String dt) {
      dts.add(dt);
    }

    String popDT() {
      return dts.remove(dts.size() - 1);
    }

    String currentDT() {
      return dts.get(dts.size() - 1);
    }
  ...
  private void initTables() throws RulesException {
    ...
    RulesAdminService admin = new RulesAdminService(
      session,
      rs.getRulesDirectory()
    );
    List tables = admin.getDecisionTables(rs.getName());

    for (Object table : tables) {
      RDecisionTable dt = admin.getDecisionTable(rs.getName(), (String) table);
      int columns = dt.getConditiontable().length > 0
        ? dt.getConditiontable()[0].length
        : 0;
  ...
  public static void main(String arg[]) throws Exception {
    ...
    try {
      Coverage c = new Coverage(
        rs,
        path + "ny-autoassign\\testfiles\\output\\"
      );
      c.compute();
