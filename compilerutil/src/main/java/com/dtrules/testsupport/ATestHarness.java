...
public abstract class ATestHarness implements ITestHarness {
  ...
  public String getOutputDirectory(RuleSet rs) {
    if (outputDirectory == null) {
      ...
      if (dir == null) {
        ...
        outputDirectory =
          rs.getRulesDirectory().getSystemPath() + dir.toString();
      }
  ...
  public File[] getFiles(RuleSet rs) {
    ...
    for (int i = 0; i < files.length - 1; i++) {
      for (int j = 0; j < files.length - 1 - i; j++) {
        if (
          files[j].getName().compareTo(files[j + 1].getName()) > 0 ||
  ...
  public void loadData(IRSession session, String path, String dataset)
    throws Exception {
    ...
    if (harnessVersion() < 2) {
      ...
      AutoDataMap autoDataMap = session
        .getRuleSet()
        .getAutoDataMap(session, mapName());
  ...
  public String runfile(
    ...
    try {
      out =
        new PrintStream(
          getOutputDirectory(rs) + number + root + "_results.xml"
        );
      if (Trace()) {
        tracefile =
          new FileOutputStream(
            getOutputDirectory(rs) + number + root + "_trace.xml"
          );
      }
      ...
      if (Verbose()) {
        if (harnessVersion() < 2) {
          datamap.print(
            new FileOutputStream(
              getOutputDirectory(rs) + number + root + "_datamap.xml"
            )
          );
        } else {
          autoDataMap.printDataLoadXML(
            new FileOutputStream(
              getOutputDirectory(rs) + number + root + "_datamap.xml"
            )
          );
        ...
        entityfile =
          new FileOutputStream(
            getOutputDirectory(rs) + number + root + "_entities_before.xml"
          );
      ...
      if (Verbose()) {
        entityfile =
          new FileOutputStream(
            getOutputDirectory(rs) + number + root + "_entities_after.xml"
          );
        RArray entitystack = RArray.newArray(session, false, false);
        for (int i = 0; i < session.getState().edepth() - 2; i++) {
          entitystack.add(session.getState().entityfetch(i));
    ...
    if (MATCH.match == v) {
      ...
      if (thisResult.getTags() != null) {
        for (int i = 0; i < thisResult.getTags().size(); i++) {
          String r = compareNodes(
            thisResult.getTags().get(i),
  ...
  public void removeIds(Node node) {
    ...
    if (
      node.getName().equals("mapping_key") &&
  ...
  public void compareTestResults(RuleSet rs) throws Exception {
    ...
    if (outputs == null || !outputs.isDirectory()) {
      ostream.println(
        "'" + getOutputDirectory(rs) + "' does not exist or is not a directory"
      );
    }
    ...
    for (File file : files) {
      if (file.getName().endsWith("_results.xml")) {
        ...
        try {
          ...
          result2 =
            XMLTree.BuildTree(
              new FileInputStream(getResultDirectory(rs) + file.getName()),
              false,
              false
            );
          if (result1 != null && result2 != null) {
            ...
            ostream.flush();
            estream.println(
              file.getName() + " has no result file; No compare done."
            );
            estream.flush();
          ...
          report.printdata(
            ...
            "file",
            file.getName(),
            "Missing Files to do the compare"
          );
        }
      }
    }
  ...
  public PrintStream compareTestResultsReport(RuleSet rs) throws Exception {
    PrintStream ctrr = new PrintStream(
      getOutputDirectory(rs) + "TestResults.xml"
    );
    return ctrr;
  }
}
