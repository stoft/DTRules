...
public class SudokuLanguage implements ICompiler {
   ...
   * @throws Exception
   */
  private void addType(IREntity entity, RName name, int itype)
    throws Exception {
    ...
    if (type == null) {
      ...
      if (type.getType() != itype) {
        ...
        throw new Exception(
          "Conflicting types for attribute " + name + " on " + entitylist
        );
      }
  ...
  public HashMap<RName, IRType> getTypes(EntityFactory ef) throws Exception {
    ...
    while (tables.hasNext()) {
      ...
      SLType type = new SLType(
        tablename,
        IRObject.iDecisiontable,
        (REntity) ef.getDecisiontables()
      );
      if (types.containsKey(tablename)) {
        System.out.println(
          "Multiple Decision Tables found with the name '" +
          types.get(tablename) +
          "'"
        );
      }
  ...
  public void printTypes(PrintStream out) throws RulesException {
    ...
    for (int i = 0; i < typenames.length - 1; i++) {
      for (int j = 0; j < typenames.length - 1; j++) {
        RName one = (RName) typenames[j], two = (RName) typenames[j + 1];
        if (
          ((SLType) types.get(one)).getType() >
          ((SLType) types.get(two)).getType()
        ) {
          Object hold = typenames[j];
          typenames[j] = typenames[j + 1];
          typenames[j + 1] = hold;
   ...
   */
  @Override
  public String compilePolicyStatement(String policyStatement)
    throws Exception {
