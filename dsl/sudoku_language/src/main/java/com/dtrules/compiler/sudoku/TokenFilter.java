...
public class TokenFilter implements Scanner {
  ...
  TokenFilter(
    ...
    Scanner scanner,
    HashMap<RName, IRType> types,
    HashMap<String, RLocalType> localtypes
  ) {
    this.types = types;
  ...
  public Symbol next_token() throws Exception {
    ...
    if (next.sym == sym.NAME && next.value.toString().charAt(0) == '$') {
    ...
    if (next.sym == sym.POSSESSIVE) {
      ...
      if (theType == -1) {
        if (localtypes.containsKey(ident.toLowerCase())) {
          ...
          throw new RuntimeException(
            "Undefined attribute '" + next.value + "'"
          );
        }
      } else {
    ...
    if (next.sym == sym.IDENT) {
      ...
      if (theType == IRObject.iEntity) {
        next.sym = (sym.RENTITY);
      } else if (theType == IRObject.iName) {
        next.sym = (sym.RNAME);
      } else if (theType == IRObject.iInteger) {
        next.sym = (sym.RLONG);
      } else if (theType == IRObject.iDouble) {
        next.sym = (sym.RDOUBLE);
      } else if (theType == IRObject.iString) {
        next.sym = (sym.RSTRING);
      } else if (theType == IRObject.iBoolean) {
        next.sym = (sym.RBOOLEAN);
      } else if (theType == IRObject.iDecisiontable) {
        next.sym = (sym.RDECISIONTABLE);
      } else if (theType == IRObject.iArray) {
        next.sym = (sym.RARRAY);
      } else if (theType == IRObject.iDate) {
        next.sym = (sym.RDATE);
      } else if (theType == IRObject.iTable) {
        next.sym = (sym.RTABLE);
      } else if (theType == IRObject.iOperator) {
        next.sym = (sym.ROPERATOR);
      } else if (theType == IRObject.iXmlValue) {
        next.sym = (sym.RXMLVALUE);
      } else if (theType == -1) {
        next.sym = (sym.UNDEFINED);
      } else {
        System.out.println("Unhandled Type");
        tokens.add(
          next.sym + " " + type2str(next.sym) + " " + next.value.toString()
        );
