...
public class DTRulesTarget implements IDataTarget {
  ...
  public IREntity findEntity(
    ...
    try {
      if (label.isSingular()) { // If singular, look for an instance
        ...
        ) { // NOTE: We are NOT allowing "" as a key here!
      ...
      if (e == null) throw new RuntimeException(
        "Failed to create the entity " + entity
      );
      if (key != null) {
        e.addAttribute(
          IREntity.mappingKey,
  ...
  public Object mapList(
    ...
    try {
      ...
      if (a.getSubType().isPrimitive()) {
        ...
        if (
          node.getData() != null
        ) for (Object d : (List<Object>) node.getData()) {
        ...
        for (IMapNode c : children) {
          ...
          if (
            list !=
            null && // If we have a list, and
            o !=
            null && //   a rules engine object
            o instanceof IRObject
  ...
  public Object mapRef(
    ...
    while (
      labelMap != null &&
      labelMap.getTargetGroup() != autoDataMap.getCurrentGroup()
    ) {
    ...
    List<Object> results = (List<Object>) node
      .getChild()
      .mapNode(autoDataMap, labelMap);
    IRObject ref = null;
  ...
  public Object mapObject(
    ...
    Label target = autoDataMap
      .getCurrentGroup()
      .findLabel(labelMap.getTarget());
    IREntity entity = findEntity(
      autoDataMap,
      target.getSpec(),
      target,
      node.getKey()
  ...
  @Override
  public Object mapAttribute(
    AutoDataMap autoDataMap,
    LabelMap labelMap,
    MapNodeAttribute node
  ) {
    RName rname = mapName(autoDataMap, labelMap, node.getAttribute().getName());
    IREntity entity = (IREntity) autoDataMap.getCurrentObject();
    if (entity == null) {
  ...
  public void init(AutoDataMap autoDataMap) {
    ...
    try {
      ...
      throw new RuntimeException(
        "Error initializing the session for accepting data. " + e
      );
    }
  }

  @Override
  public Object mapMap(
    AutoDataMap autoDataMap,
    LabelMap labelMap,
    MapNodeMap node
  ) {
    ...
    try {
      ...
      if (rTable == null) {
        rTable =
          RTable.newRTable(
            autoDataMap.getSession().getEntityFactory(),
            null,
            null
          );
      }
