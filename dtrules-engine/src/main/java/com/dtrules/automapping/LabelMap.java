...
public class LabelMap {
  ...
  private final String source; // The source label
  private final String target; // The target label
  ...
  public LabelMap(AutoDataMapDef autoDataMapDef, String source, String target) {
    this.source = source;
    this.target = target;
    ...
    if (sourceGroup == null || targetGroup == null) {
      String badones =
        (sourceGroup == null ? " '" + source + "' " : " ") +
        (targetGroup == null ? "'" + target + "' " : " ");
      throw new RuntimeException("Undefined Labels(s):" + badones);
    }
  }

  public static LabelMap newLabelMap(
    AutoDataMapDef autoDataMapDef,
  ...
  public void printXML(XMLPrinter xout) {
    xout.opentag("mapobject", "source", source, "target", target);
