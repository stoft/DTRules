...
public class RArrayOps {
    ...
    public void execute(DTState state) throws RulesException {
      ...
      for (int i = 0; i < size - 1; i++) {
        for (int j = 0; j < size - 1 - i; j++) {
          if (
            ((IRObject) array.get(j + 1)).compare((IRObject) array.get(j)) ==
            direction
          ) {
  ...
  public static class Merge extends ROperator {
    ...
    public void execute(DTState state) throws RulesException {
      ...
      state.datapush(
        RArray.newArray(state.getSession(), false, newarray, false)
      );
    }
  }
    ...
    public void execute(DTState state) throws RulesException {
      ...
      for (IRObject v : array1) {
        if (array2.contains(v)) {
          ret.add(v);
          if (state.testState(DTState.TRACE)) {
            state.traceInfo("addto", "arrayId", ret.getID() + "", v.postFix());
          }
        }
      }
