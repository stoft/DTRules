...
public class RArray extends ARObject implements Collection<IRObject> {
  ...
  public static RArray newArray(
    IRSession session,
    boolean duplicates,
    boolean executable
  ) throws RulesException {
    RArray ar = new RArray(
      session.getEntityFactory().getUniqueID(),
      duplicates,
      executable
    );
    if (session.getState().testState(DTState.TRACE)) {
      session
        .getState()
        .traceInfo("newarray", "arrayId", ar.getID() + "", null);
    }
    return ar;
  }
  ...
  public static RArray newArray(
    ...
    if (session.getState().testState(DTState.TRACE)) {
      ...
      for (IRObject v : thearray) {
        session
          .getState()
          .traceInfo("addto", "arrayId", ar.getID() + "", v.postFix());
      }
    }
    return ar;
  }
   ...
   * @throws RulesException
   */
  public static RArray newArrayTraceInterface(
    int id,
    boolean duplicates,
    boolean executable
  ) throws RulesException {
