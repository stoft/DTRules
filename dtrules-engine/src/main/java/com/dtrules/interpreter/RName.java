...
public class RName extends ARObject implements Comparable<RName> {
   ...
   */
  public static RName getRName(String _name) {
    String cache = _name;
   ...
   * @return
   */
  public static RName getRName(String _name, boolean _executable) {
    // Fix the name; trim and then replace internal spaces with '_'.
  ...
  public static RName getRName(
    ...
    if (rn != null && _entity == null) {
      return _executable
        ? (RName) rn.getExecutable()
        : (RName) rn.getNonExecutable();
    }
    if (
      rn != null
    ) // Fix the name; trim and then replace internal spaces with '_'.
