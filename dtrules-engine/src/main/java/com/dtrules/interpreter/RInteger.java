...
public class RInteger extends ARObject {
   ...
   */
  public static long getIntegerValue(String s) throws RulesException {
    if (
      s == null || (s = s.trim()).length() == 0 || s.equalsIgnoreCase("null")
    ) {
      return 0L;
    }
    try {
      ...
      throw new RulesException(
        "Conversion Error",
        "RInteger.getIntegerValue()",
        "Could not covert the string '" +
        s +
        "' to an integer: " +
        e.getMessage()
      );
  ...
  public static RInteger getRIntegerValue(String s) throws RulesException {
    return getRIntegerValue(getIntegerValue(s));
  }

  public static RInteger getRIntegerValue(long i) {
    if (i == -1) return mone;
    if (i == 0) return zero;
    if (i == 1) return one;
  ...
  public static RInteger getRIntegerValue(double i) {
  ...
  public int compare(IRObject irObject) throws RulesException {
    return (this.value == irObject.intValue())
      ? 0
      : ((this.value < irObject.intValue()) ? -1 : 0);
  }
}
