...
public class RTable extends ARObject {
  ...
  private RTable(EntityFactory ef, RName tablename, String description)
   ...
   * @return
   */
  public static RTable newRTable(
    EntityFactory ef,
    RName tablename,
    String description
  ) throws RulesException {
   ...
   * @param values
   */
  public void setValues(IRSession session, String values)
    throws RulesException {
   ...
   * @throws RulesException
   */
  public void setValue(DTState state, IRObject[] keys, IRObject value)
    throws RulesException {
    IRObject v = this;
    for (int i = 0; i < keys.length - 1; i++) {
      if (v.type() != type) {
        throw new RulesException(
          "OutOfBounds",
          "RTable",
          "Invalid Number of Keys used with Table " + this.stringValue()
        );
      }
      ...
      if (next == null) {
        next =
          newRTable(
            state.getSession().getEntityFactory(),
            this.tablename,
