...
public class REntity extends ARObject implements IREntity {
   ...
   *
   */
  public REntity(boolean _readonly, REntity entity, IRSession s)
   ...
   */
  public void put(IRSession session, RName attrib, IRObject value)
    throws RulesException {
    REntityEntry entry = (REntityEntry) attributes.get(attrib);
    if (entry == null) throw new RulesException(
      "Undefined",
      "REntity.put()",
      "Undefined Attribute " + attrib + " in Entity: " + name
    );
    if (
      value.type().getId() != iNull &&
      entry.type.getId() != value.type().getId()
    ) {
      int entrytype = entry.type.getId();
      if (entrytype == iInteger) value = value.rIntegerValue(); else if (
        entrytype == iDouble
      ) value = value.rDoubleValue(); else if (entrytype == iBoolean) value =
        value.rBooleanValue(); else if (entrytype == iEntity) value =
        value.rEntityValue(); else if (entrytype == iName) value =
        value.rNameValue(); else if (entrytype == iString) value =
        value.rStringValue(); else if (entrytype == iDate) value =
        value.rTimeValue(session);
    }
