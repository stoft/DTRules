...
public class CNode implements DTNode {
  ...
  public boolean equalsNode(DTState state, DTNode node) {
    if (node instanceof CNode) {
      ...
      return (
        cnode.iffalse.equalsNode(state, iffalse) &&
  ...
  public ANode getCommonANode(DTState state) {
    if (
      state.testState(DTState.TRACE) && // If we are tracing, don't do this.
      iftrue.equalsNode(state, iffalse)
    ) { // If both true/false paths are the same,
    ...
    try {
      if (state.testState(DTState.TRACE)) {
        if (state.testState(DTState.VERBOSE)) {
          ...
          state.traceInfo(
            "Postfix",
            dtable.getConditionsPostfix()[conditionNumber]
          );
      ...
      RulesException re = new RulesException(
        e.getClass().getName(),
  ...
  public void addNode(DTNode _node) {
    if (!(_node instanceof CNode)) {
      throw new RuntimeException(
        "Shouldn't ever attempt to combine nodes of different types"
      );
    }
