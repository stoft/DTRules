...
public class RDecisionTable extends ARObject {
  ...
  private void whatsUsed() {
    ...
    for (int col = 0; col < conditionCnt; col++) { // For each column
      for (int row = 0; row < conditiontable.length; row++) { // For each row
        if (
          conditiontable[row][col].equalsIgnoreCase("y") ||
      ...
      for (int row = 0; row < actions.length; row++) {
        if (
          columnsSpecified[col] && actiontable[row][col].equalsIgnoreCase("x")
        ) {
  ...
  public RDecisionTable(IRSession session, String name) throws RulesException {
    ...
    if (dttable != null) {
      new CompilerError(
        CompilerError.Type.TABLE,
        "Duplicate Decision Tables Found",
        0,
        0
      );
    }
  ...
  public List<IDecisionTableError> compile() {
    try {
      ...
      for (int i = 0; i < initialActions.length; i++) {
        try {
          rinitialActions[i] =
            RString.compile(session, initialActionsPostfix[i], true);
        } catch (Exception e) {
          errorlist.add(
            new CompilerError(
      ...
      for (int i = 0; i < ractions.length; i++) {
        try {
          ...
          if (
            ractions[i] != null &&
            type == Type.ALL &&
            callsPolicyStatement(ractions[i], null)
          ) {
            optimize = false;
          }
        } catch (RulesException e) {
      ...
      for (int i = 0; i < policystatementsPostfix.length; i++) {
        try {
          rpolicystatements[i] =
            RString.compile(session, policystatementsPostfix[i], true);
        } catch (RulesException e) {
          errorlist.add(
            new CompilerError(
    ...
    try {
      ...
      if (
        ...
        throw new RulesException(
          "Stacks Not balanced",
          "DecisionTables",
          "Error while executing table: " +
          getName().stringValue() +
          "\n" +
          (
            estk != state.edepth()
              ? "Entity Stack size before: " +
              estk +
              " after: " +
              state.edepth() +
              "\n"
  ...
  public void executeTable(DTState state) throws RulesException {
    ...
    if (compiled == false) {
      throw new RulesException(
        ...
        "Attempt to execute an uncompiled decision table: " +
        dtname.stringValue()
      );
    }
    ...
    if (trace) {
      ...
      for (
        int i = 0;
        rinitialActions != null && i < rinitialActions.length;
        i++
      ) {
        try {
  ...
  void buildBalanced() {
    if (
      ...
      conditiontable[0][0].equals("*")
    ) { // If *, we just execute all actions
    ...
    if (rowCol != null) {
      errorlist.add(
        new CompilerError(
          IDecisionTableError.Type.TABLE,
          "Condition Table isn't balanced.",
          rowCol.row,
          rowCol.col
        )
      );
  ...
  public void writeXML(PrintStream p) {
    ...
    for (int i = 0; i < conditions.length; i++) {
      ...
      printattrib(
        ...
        GenericXMLParser.encode(conditions[i])
      );
      printattrib(
        p,
        "condition_postfix",
        GenericXMLParser.encode(conditionsPostfix[i])
      );
      printattrib(
        p,
        "condition_comment",
        GenericXMLParser.encode(conditionsComment[i])
      );
      p.println();
      newline = true;
      for (int j = 0; j < maxcol; j++) {
        p.println(
          "<condition_column column_number=\"" +
          (j + 1) +
          "\" column_value=\"" +
          conditiontable[i][j] +
          "\" />"
    ...
    for (int i = 0; i < actions.length; i++) {
      ...
      printattrib(
        p,
        "action_postfix",
        GenericXMLParser.encode(actionsPostfix[i])
      );
      printattrib(
        p,
        "action_comment",
        GenericXMLParser.encode(actionsComment[i])
      );
      p.println();
      newline = true;
      for (int j = 0; j < maxcol; j++) {
  ...
  public void insert(int col) throws RulesException {
    if (maxcol >= 16) {
      throw new RulesException(
        "TableTooBig",
        "insert",
        "Attempt to insert more than 16 columns in a Decision Table"
      );
    }
  ...
  private DTNode processCol(
    ...
    if (starColumn >= 0 && col > starColumn) {
      ...
      errorlist.add(
        new CompilerError(
          IDecisionTableError.Type.TABLE,
          "Only one 'star' column is allowed, and it must be the last column.",
          "*",
          rowX
        )
      );
    ...
    if (row >= conditions.length) { // Ah, end of the column!
      ...
      if (istar >= 0) {
        ...
        boolean otherwise =
          conditions[istar].trim().equalsIgnoreCase("otherwise") ||
        ...
        if (!always && !otherwise) {
          errorlist.add(
            new CompilerError(
              ...
              " of 'default', 'otherwise', or 'always'",
              "*",
              0
            )
          );
    ...
    if (yes || dcare) { // If 'y' or a don't care,
      ...
      if (yes && t.getStar()) {
        errorlist.add(
          new CompilerError(
            IDecisionTableError.Type.CONDITION,
            "Cannot follow a 'Y' with a '*' at row " +
            (row + 1) +
            " column " +
            (col + 1),
            v,
    ...
    if (no || dcare) { // If 'n' or a don't care,
      ...
      if (no && t.getStar()) {
        errorlist.add(
          new CompilerError(
            IDecisionTableError.Type.CONDITION,
            "Cannot follow a 'N' with a '*' at row " +
            (row + 1) +
            " column " +
            (col + 1),
            v,
  ...
  private void addlist(
    ArrayList<RDecisionTable> calledTables,
    IRObject[] list
  ) {
   ...
   * @return
   */
  private ArrayList<RDecisionTable> getTables(
    ArrayList<RArray> stack,
    ArrayList<RDecisionTable> tables,
    IRObject obj
  ) {
