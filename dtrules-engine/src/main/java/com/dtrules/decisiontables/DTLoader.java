...
public class DTLoader implements IGenericXMLParser {
  ...
  public void end_decision_tables() throws Exception {
    ...
    while (it.hasNext()) {
      try {
        ...
        state.traceInfo(
          "error",
          "Error building Decision Table " + dt + "\r\n" + e
        );
      }
    }
  }
  ...
  public void end_decision_table() {
    ...
    for (int i = 0; i < c_cnt; i++) {
      ...
      dt.conditionsComment[i] =
        c_comment.size() <= i || c_comment.get(i) == null
          ? ""
          : c_comment.get(i);
      dt.conditionsPostfix[i] =
        c_postfix.size() <= i || c_postfix.get(i) == null
          ? ""
          : c_postfix.get(i);
    ...
    for (int i = 0; i < a_cnt; i++) {
      ...
      dt.actionsComment[i] =
        a_comment.size() <= i || a_comment.get(i) == null
          ? ""
          : a_comment.get(i);
      dt.actionsPostfix[i] =
        a_postfix.size() <= i || a_postfix.get(i) == null
          ? ""
          : a_postfix.get(i);
      for (int j = 0; j < col_cnt; j++) {
    ...
    if (_tag.equalsIgnoreCase("type")) {
      if (_body.equalsIgnoreCase("first")) {
        ...
        throw new RulesException(
          "Invalid",
          "Decision Table Load",
          "Bad Decision Table type Encountered: '" + _body + "'"
        );
      }
    }
  }
  ...
  public void begin_action_details() {
    for (int i = 0; i < col_cnt; i++) {
      ...
      if (a_table.get(a_cnt) == null) a_table.set(
        a_cnt,
  ...
  public void begin_policy_statement() {
    ...
    if (ps_col < 0 || ps_col > col_cnt) {
      throw new RuntimeException(
        "The column value for a Policy Statement '" +
        ps_col +
        "' is out of range"
      );
    }
  ...
  public void beginTag(
    ...
    try {
    ...
    } catch (NoSuchMethodException e) {} catch (InvocationTargetException e) { // Ignore undefined tags	 // Errors thrown by Decision Table parsing
      throw new RuntimeException(
        "An Invocation Target Exception was thrown processing the Begin XML tag " +
        tag +
        "\nError states: " +
        e.getCause()
      );
      ...
      throw new RuntimeException(
        "Error Parsing Decision Tables at begin tag: " +
        tag +
        "\r\n " +
        e.getMessage()
      );
  ...
  public void endTag(
    ...
    try {
      ...
      state.traceInfo(
        "error",
        "An Invocation Target Exception was thrown processing the End XML tag " +
        tag +
        "\nError states " +
        e.getCause()
      );
      ...
      state.traceInfo("error", e.getCause().getMessage());
      throw new RuntimeException(
        "Error Parsing Decision Tables at end tag: " +
        tag +
        " body: " +
        body +
        "\r\n" +
