...
public class LoadDatamapData extends LoadXMLData {
    ...
    if (attr != null) {
      ...
      if (body != null) {
        ...
        if (enclosingEntity == null) {
          throw new Exception(
            "No Entity is in the context that defines " + a.stringValue()
          );
        }
        ...
        if (type == IRObject.iInteger) {
          ...
          if (xmltag != null) {
            ...
            throw new RulesException(
              "MappingError",
              "LoadDatamapData",
              "Somehow we are missing the XML Tag for the attribute: " + a
            );
          }
        } else {
          throw new RulesException(
            "MappingError",
            "LoadDatamapData",
            "Unsupported type encountered: " + enclosingEntity.getEntry(a).type
          );
        }
  ...
  long getLong(Object num) {
    if (num.getClass() == BigDecimal.class) return (
      (BigDecimal) num
    ).longValue();
  ...
  double getDouble(Object num) {
    ...
    if (num.getClass() == String.class) return Double.parseDouble(
      num.toString()
    );
    throw new RuntimeException(
      "Can't figure out the value " +
  ...
  IREntity UpdateReferences(IREntity e) throws RulesException {
    ...
    for (int i = 0; i < state.edepth(); i++) {
      if ((state.getes(i)).get(e.getName()) != null) {
        ...
        ) (state.getes(i)).put(null, e.getName(), e); // Update a reference to an Entity of the same name, //  but only if it isn't a self reference.
      }
    }
