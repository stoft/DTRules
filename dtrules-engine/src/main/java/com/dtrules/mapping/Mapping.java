...
public class Mapping {
    ...
    for (String entity : entitystack) {
      try {
        ...
        throw new RuntimeException(
          "Failed to initialize the Entity Stack: " + e
        );
      }
    }
  }
  ...
  public static Mapping newMapping(
    ...
    try {
      InputStream s = session
        .getRulesDirectory()
        .getFileSource()
        .openStreamSearch(FileType.MAP, session.getRuleSet(), filename);
   ...
   */
  public void loadStringData(IRSession session, String Source, String data)
    throws RulesException {
    Reader strReader = new StringReader(data);
    loadData(session, strReader, Source);
  }

  public void loadData(IRSession session, String dataSource)
    throws RulesException {
    loadData(session, dataSource, dataSource);
  }

  public void loadData(IRSession session, String dataSource, String source)
    throws RulesException {
    InputStream input = session
      .getRulesDirectory()
      .getFileSource()
      .openStreamSearch(FileType.MAP, session.getRuleSet(), dataSource);
    if (input == null) {
      throw new RulesException(
        "File Not Found",
        "Mapping.loadData()",
        "Could not open " + dataSource
      );
    }
  ...
  public void loadData(IRSession session, Reader dataSource, String source)
    throws RulesException {
    if (dataloader == null) {
      dataloader =
        new LoadXMLData(this, session, session.getRuleSet().getName());
    }
    if (source == null) {
      source = "XML file";
    }
   ...
   * @throws RulesException
   */
  public void loadData(IRSession session, DataMap datasrc)
    throws RulesException {
    if (dataloader == null) {
      dataloader =
        new LoadDatamapData(this, session, session.getRuleSet().getName());
    }
    try {
      if (session.getState().testState(DTState.TRACE)) {
  ...
  void processTag(XMLNode tag) throws Exception {
    if (tag.type() == XMLNode.Type.TAG) {
      state.traceTagBegin(
        tag.getTag() == null ? "process" : tag.getTag(),
        null
      );
      ...
      ((LoadDatamapData) dataloader).endTag(
          tagstk,
          tagstkptr,
          tag,
          tag.getBody(),
