...
public class LoadMapping implements IGenericXMLParser {
  ...
  public LoadMapping(Mapping _map, IRSession session, String _ruleSetName) {
    ...
    while (es.hasNext()) {
      ...
      try {
        ...
        state.traceInfo(
          "error",
          "Failed to initialize the Entity Stack (Failed on " +
          ename +
          ")\n" +
          e
        );
   ...
   */
  IREntity findEntity(String entity, String code, EntityInfo info)
    throws RulesException {
    ...
    if (number.equals("1")) {
      ...
      if (e == null) {
        e =
          session.getState().findEntity(RName.getRName(entity + "." + entity));
        if (e == null) {
          ...
          if (type == IRObject.iInteger) {
            ...
            if (entity != null) {
              ...
              throw new RulesException(
                "MappingError",
                "LoadMapping",
                "Entity Tags have to create some Entity Reference"
              );
            }
          } else {
    ...
    switch (attrib.type) {
      ...
      default:
        ...
        throw new RuntimeException(
          "Bad Type Code " +
          type +
          " in com.dtrules.mapping.AttributeInfo: " +
          attrib.rAttribute
        );
    }
  }
  ...
  IREntity UpdateReferences(IREntity e, EntityInfo info) throws RulesException {
    ...
    if (((IREntity) state.getes(i)).get(e.getName()) != null) {
      ...
      if (
        !refto.getName().equals(e.getName())
      ) ((IREntity) state.getes(i)).put(null, e.getName(), e); // Update a reference to an Entity of the same name, //  but only if it isn't a self reference.
    }
