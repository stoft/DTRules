...
class LoadMap implements IGenericXMLParser {
  ...
  public void begin_do2entitymap() {
    ...
    for (DataObjectMap dataobjmap : dataobjmaps) {
      if (
        dataobjmap.entityName.equals(entity) &&
        dataobjmap.tag.equals(entity_tag)
      ) {
        throw new RuntimeException(
          "Duplicate DO to Entity mappings for: " + entity
        );
      }
    }

    try {
      DataObjectMap dataobjmap = new DataObjectMap(
        doClass,
        entity,
        entity_tag,
        key,
  ...
  public void begin_entity() {
    ...
    IREntity rEntity = map
      .getSession()
      .getEntityFactory()
      .findRefEntity(RName.getRName(entity));
    if (rEntity == null) {
      System.out.println(
        "The Entity specified, '" + entity + "' is not defined"
      );
  ...
  public void begin_createentity() {
    ...
    try {
      ...
      System.out.println(
        "\nThe Entity found in the Map File => " +
        entity +
        " <= isn't defined by the EDD."
      );
      loadSuccessful = false;
    }
    ...
    try {
      ...
      if (e == null) {
        if (!undefinedEntities.containsKey(entity)) {
          System.out.println(
            "The entity " + entity + " isn't defined in the EDD"
          );
          loadSuccessful = false;
        ...
        if (e.getEntry(RName.getRName(rattribute)) == null) {
          System.out.println(
            "The Attribute " + rattribute + " isn't defined by " + entity
          );
          loadSuccessful = false;
        }
  ...
  public void beginTag(
    ...
    try {
      this.getClass()
        .getMethod("begin_" + tag, (Class[]) null)
