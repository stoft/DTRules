...
public class DTState {
  ...
  public void traceInfo(
    ...
    if (testState(TRACE)) out.printdata(
      tag,
      name1,
      value1,
      name2,
   ...
   * @param value
   */
  public boolean def(RName name, IRObject value, boolean protect)
    throws RulesException {
    ...
    for (int i = entitystkptr - 1; i >= 0; i--) {
      ...
      if (
        !e.isReadOnly() &&
        (entityname == null || e.getName().equals(entityname))
      ) {
  ...
  public String toString() {
    ...
    for (int i = 0; i < entitystkptr; i++) {
      s +=
        entitystk[i].getName().stringValue() +
        "[" +
        entitystk[i].getID() +
        "] ";
    }
