...
public class RSession implements IRSession {
    ...
    while (ie.hasNext()) {
      ...
      if (entityInstances.containsKey(id)) {
        throw new RulesException(
          ...
          e.getName() +
          " and " +
          entityInstances.get(id).getName()
        );
      }
  ...
  public void dump(REntity e) throws RulesException {
    ...
    dtstate.traceTagBegin(
      "entity",
      "name",
      e.getName().stringValue(),
      "id=",
      e.getID() + ""
    );
    dump(e, 1);
        ...
        if (type == IRObject.iEntity) {
          if (value.type().getId() == IRObject.iNull) {
            ...
            dtstate.traceTagBegin(
              "entity",
              "name",
              ((REntity) value).getName().stringValue(),
              "id",
              printIds ? ((REntity) value).getID() + "" : ""
            );

            if (
              !(boundries.get(e) != null && boundries.get(e).contains(value))
            ) {
              dtstate.debug(" recurse\n");
            } else {
              if (boundries.get(e) == null) boundries.put(
                e,
  ...
  public void initialize(String entrypoint) throws RulesException {
    ...
    if (entities == null) {
      throw new RulesException(
        "undefined",
        "executeAt",
        "The entry point '" + entrypoint + "' is undefined"
      );
    }
  ...
  public IREntity createEntity(Object id, RName name) throws RulesException {
    ...
    if (ref == null) {
      throw new RulesException(
        ...
        "An attempt ws made to create the entity " +
        name.stringValue() +
        "\n" +
  ...
  public void printEntity(IXMLPrinter rpt, String tag, IREntity e)
    throws Exception {
    ...
    for (RName name : keys) {
      ...
      if (
        v.type().getId() == IRObject.iArray && v.rArrayValue().size() == 0
      ) continue;
  ...
  public void printBalancedTables(PrintStream out) throws RulesException {
    Iterator<RName> dts =
      this.getEntityFactory().getDecisionTableRNameIterator();
    while (dts.hasNext()) {
