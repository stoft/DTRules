...
public class EDDLoader implements IGenericXMLParser {
  ...
  public void beginTag(
    ...
    if (tag.equals("entity_data_dictionary")) {
      ...
      } catch (NullPointerException e) {} catch (Exception e) {} // Ignore any errors
    } else if (version == 2) {
    ...
    if (version == 2) {
      ...
      if (!writeable && !readable) {
        errorMsgs +=
          "\nThe attribute " +
          attribute +
          " has to be either readable or writable\r\n";
        succeeded = false;
        rtype = RNull.type;
      }
      ...
      try {
        defaultO =
          session
            .getComputeDefault()
            .computeDefaultValue(session, ef, defaultv, rtype);
      } catch (RulesException e) {
        errorMsgs +=
          "Bad Default Value '" +
          defaultv +
          "' encountered on entity: '" +
          entityname +
          "' attribute: '" +
          attribute +
          "' \n";
      ...
      String errstr = entity.addAttribute(
        attributeRName,
        defaultv,
        defaultO,
        writeable,
  ...
  public void endTag2(
    ...
    IRObject defaultO = session
      .getComputeDefault()
      .computeDefaultValue(session, ef, default_value, rtype);
