...
public class RuleSet {
  ...
  public AutoDataMap getAutoDataMap(IRSession session, String name)
    ...
    if (!mapDefinitions.containsKey(name)) {
      ...
      if (mapfilename != null) {
        mapStream =
          rd.streamSource.openStreamSearch(
            FileType.AUTO_MAP,
            this,
            mapfilename
          );
  ...
  public void setDefaultCompiler(String qualifiedCompilerClassName) {
    try {
      this.defaultCompiler =
        (Class<ICompiler>) Class.forName(qualifiedCompilerClassName);
   ...
   * @throws RulesException
   */
  public synchronized IRSession newSession() throws RulesException {
   ...
   * @throws RulesException
   */
  protected EntityFactory getEntityFactory(IRSession session)
    throws RulesException {
    return getEntityFactory(ef == null, session);
  }

  protected EntityFactory getEntityFactory(boolean load, IRSession session)
    throws RulesException {
    if (load) {
      synchronized (this) {
        ...
        while (iedds.hasNext()) {
          ...
          InputStream s = rd.streamSource.openStreamSearch(
            FileType.EDD,
            this,
            filename
          );
          if (s == null) {
            if (s == null) {
              System.out.println(
                ...
                filename +
                "\r\n   WorkingDirectory: " +
                session.getRuleSet().getWorkingdirectory() +
                "\r\n   ResourcePath:     " +
                session.getRuleSet().getResourcepath() +
                "\r\n   SystemDirecotry:  " +
                session.getRuleSet().getSystemPath()
              );
            }
          }
        ...
        while (idts.hasNext()) {
          ...
          InputStream s = rd.streamSource.openStreamSearch(
            FileType.DECISIONTABLES,
            this,
            filename
          );
          if (s == null) {
            System.out.println(
              "No Decision Table XML found" +
              "\r\n   Looking for:      " +
              filename +
              "\r\n   WorkingDirectory: " +
              session.getRuleSet().getWorkingdirectory() +
  ...
  public String getDT_XMLName() throws RulesException {
    if (dt_names.size() != 1) {
      throw new RulesException(
        ...
        "RuleSet",
        "We can only have one DecisionTable XML file"
      );
    }
  ...
  public String getEDD_XMLName() throws RulesException {
    if (edd_names.size() != 1) {
      throw new RulesException(
        "UnsupportedConfiguration",
        "RuleSet",
        "We can only have one EDD XML file"
      );
    }
  ...
  public List<IRObject> getDecisionTables() throws RulesException {
    List<IRObject> tables = getEntityFactory(newSession())
      .getDecisiontables()
      .getValues();
    return tables;
  }
