...
public class TestCaseGen {
  ...
  int genClient(int ageLimit) {
    ...
    xout.printdata(
      "expectedChildren",
      pregnant && chance(80) ? 1 : pregnant && chance(80) ? 2 : pregnant ? 3 : 0
    );
    xout.printdata(
      "validatedImmigrationStatus",
      ((!citizen) && chance(50)) ? "true" : "false"
    );
    xout.printdata("disabled", chance(10) ? "true" : "false");
    xout.printdata(
      "applying",
      age < 22 ? (chance(90) ? "true" : "false") : "false"
    );
    ...
    try {
      ...
      for (int i = 1; i <= numCases; i++) {
        ...
        xout =
          new XMLPrinter(
            "chip_case",
            new FileOutputStream(filename(name, numCases, i))
          );
        generate();
        xout.close();
      }
  ...
  public static void main(String args[]) {
    ...
    if (args.length > 1) tcg.path = args[1];
